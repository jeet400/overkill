...
if (config.mode === 'production' || false) {
  app.use((req, res, next) => {
    ...
    res.setHeader('Hydren-Product', 'OverKill');   // Changed header here
...
// Log the ASCII and version, changed "OverSee" to "OverKill"
...
/**
 * Serve static files and start server with updated log message
 */
app.use(express.static('public'));
app.listen(config.port, () => log.info(`OverKill is listening on port ${config.port}`)); // Changed here too

app.get('*', async function(req, res){
  res.render('errors/404', {
    req,
    name: await db.get('name') || 'OverKill',  // Changed default name here
